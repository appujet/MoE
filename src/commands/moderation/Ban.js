...
module.exports = class Ban extends Command {
  ...
  async run(ctx, args) {
    ...
    if (user.id === ctx.author.id)
      return await ctx.sendMessage("You can't ban yourself.");
    if (user.id === ctx.client.user.id)
      return await ctx.sendMessage("You can't ban me.");
    if (user.id === ctx.guild.ownerId)
      return await ctx.sendMessage("You can't ban the owner.");
    if (ctx.member.roles.highest.position < user.roles.highest.position)
      return await ctx.sendMessage(
        "You can't ban that user! He/She has a higher role than yours."
      );
    if (
      user.roles.highest.position >= ctx.guild.members.me.roles.highest.position
    )
      return await ctx.sendMessage(
        "I can't ban that user! He/She has a higher role than mine."
      );
    if (user.id === this.client.user.id)
      return await ctx.sendMessage("Please don't ban me!");
